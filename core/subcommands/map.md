...
Then run the `pointer` subcommand logic for `{name}` so the `CLAUDE.local.md` graph line flips from
"not built" to the cache path + query command immediately, instead of waiting for the next
`/brain end`. Skip on `--vault-only` / no repo.
